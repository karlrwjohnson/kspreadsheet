...
import CellController, { ControlledElement } from './CellController';
...
export default class TableController extends Observable {
  ...
  _focusedCellController: CellController | null = null;

  element: HTMLElement;
  rowContainer: HTMLTableSectionElement;
  ...
  _on_navigate_north (origin) {
    ...
    if (targetController) {
      ...
      const myFoo : Foo<number> = { foo: 7 };
      const myFooBar : FooBar<number> = myFoo;

      // Prune empty rows at the bottom of the table
      if (parentElement.nextSibling === null &&
          Fn.all<Cell>(this.model.getCellsInRow(this.model.height - 1), cell => cell.isEmpty())) {
        this.model.height --;
      }
    }
  }
  ...
  focus () {
    ...
    (<ControlledElement> this.rowContainer.children[0].children[0]).controller.focused = true;
  }
